...
resource "aws_sfn_state_machine" "document_processing" {
  ...
  definition = templatefile("${path.module}/statemachine/workflow.asl.json.tftpl", {
    ...
    RuleValidationFunctionArn           = var.enable_rule_validation ? aws_lambda_function.rule_validation[0].arn : "arn:${data.aws_partition.current.partition}:lambda:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:function:nonexistent-rule-validation-function"
    EnableRuleValidation                = var.enable_rule_validation
